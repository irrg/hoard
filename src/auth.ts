...
import { CONFIG_PATH, type HoardConfig, type Storefront, readConfig, writeConfig } from './config.js';
...
async function configureItchio(config: HoardConfig): Promise<void> {
  config.HOARD_ITCHIO_USERNAME = await promptText('itch.io username:', config.HOARD_ITCHIO_USERNAME);
...
async function configureHumblebundle(config: HoardConfig): Promise<void> {
  const sess = await promptPassword('Humble Bundle _simpleauth_sess cookie (press Enter to keep existing):');
